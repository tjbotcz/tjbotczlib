...
TJBot.prototype._createServiceAPI = function(service, credentials) {
    ...
    switch (service) {
        ...
        //assert((credentials.hasOwnProperty('username') && credentials.hasOwnProperty('password')) || credentials.hasOwnProperty('iam_apikey'), "credentials for the " + service + " service missing");
        ...
        //assert(credentials.hasOwnProperty('password'), "credentials for the " + service + " service missing 'password'");
            ...
            this._toneAnalyzer = new ToneAnalyzerV3({
                ...
                throw new Error(
                'Missing authentication credentials for tone analyzer service: username/password or ' +
                'apikey are required.');
            }
        ...
        case 'visual_recognition':
            assert(credentials.hasOwnProperty('api_key') || credentials.hasOwnProperty('iam_apikey'), "credentials for the " + service + " service missing 'api_key' or 'iam_apikey'");
 ...
 */
TJBot.prototype.sessionId = function(workspaceId, callback){

        this._assistant.createSession({
            assistantId: workspaceId
          })
            .then(response => {
            var session = response.result.session_id; //JSON.stringify(response.result, null, 2); 
            console.log(session);
            callback(session);
            })
            .catch(err => {
...
TJBot.prototype.converse = function(workspaceId, session, message, context, callback) {
    this._assertCapability('converse');
...
TJBot.prototype.recognizeObjectsInPhoto = async function(filePath, credentials) {
    ...
    this._assertCapability('see');
    assert(credentials.hasOwnProperty('iam_apikey'), "credentials for the visual recognition service missing API Key.");
    assert(credentials.hasOwnProperty('apiSecret'), "credentials for the visual recognition service missing API Secret.");
 ...
 * return object.
 * 
 * DEPRECATED VisualRecognitionV3 stopped to support recognizeText function 
